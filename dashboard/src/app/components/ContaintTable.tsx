...
export default function ContaintTable() {
    ...
    const fetchData = useCallback(async (view: ViewType, isBackgroundUpdate = false) => {
        ...
        try {
            ...
            setData(fetchedData); setIsDbConnected(true); setLastDataUpdateTimestamp(new Date());
            if (!isBackgroundUpdate) setError(null);
        } catch (err) {
            setIsDbConnected(false); setError(err instanceof Error ? err.message : "Ein unbekannter Fehler ist beim Laden der Daten aufgetreten.");
        } finally {
            if (!isBackgroundUpdate) setIsLoadingData(false);
        }
    }, [token, logout]);
    ...
    const filteredData = useMemo(() => {
        ...
        if (appliedStartDate || appliedEndDate) {
            ...
            tempData = tempData.filter(item => {
                let dateStringToFilter: string | null = item.erstelltam; // Default to erstelltam
    ...
    const handleCopyToClipboard = async (textToCopy: string, cellKey: string) => {
        ...
        try {
            await navigator.clipboard.writeText(textToCopy); setCopiedCellKey(cellKey);
            setTimeout(() => setCopiedCellKey(null), 1500);
        } catch (err) { setError("Kopieren fehlgeschlagen."); setTimeout(() => setError(null), 3000); }
    };
    ...
    const performStatusChangeAsync = useCallback(async (itemId: number, newStatus: AnyItemStatus, viewForApi: ViewType) => {
        ...
        setData(prevData => prevData.map(item => item.id === itemId && 'status' in item ? { ...item, status: newStatus as any } : item));
        setError(null);
        const apiEndpoint = API_ENDPOINTS[viewForApi];
        if (!apiEndpoint) {
            setData(originalData); setError(`API Endpunkt für ${VIEW_TITLES[viewForApi]} nicht definiert.`); return;
        }
        try {
            ...
            if (!response.ok) {
                setData(originalData); const errorData = await response.json().catch(() => ({}));
            ...
            fetchData(currentView, true); // Refetch to get updated data including 'abgeschlossenam'
        } catch (err) {
            setData(originalData); setError(err instanceof Error ? err.message : `Statusupdate für ${VIEW_TITLES[viewForApi]} fehlgeschlagen.`);
        }
    }, [token, data, logout, fetchData, currentView, setData, setError]);
    ...
    if (isLoadingAuth) { /* ... Ladeanzeige Authentifizierung ... */ }
    if (!isAuthenticated) { /* ... Login-Aufforderung ... */ }
                ...
                {currentView === "admin" ? ( <AdminSection /> ) :
                 ...
                 !isLoadingData && filteredData.length === 0 && !error ? (
                    <div className="text-center py-10 text-neutral-500">
                        ...
                        {(currentView === "beschwerden" || currentView === "lob" || currentView === "anregungen") && activeStatusFilter !== "alle" && `${(searchTerm.trim() || emailSearchTerm.trim() || idSearchTerm.trim() || (currentView === "beschwerden" && (haltestelleSearchTerm.trim() || linieSearchTerm.trim()))) ? ' und' : ''} Status "${FILTER_LABELS[activeStatusFilter]}"`}
                        {(appliedStartDate || appliedEndDate) && `${(searchTerm.trim() || emailSearchTerm.trim() || idSearchTerm.trim() || ((currentView === "beschwerden" || currentView === "lob" || currentView === "anregungen") && activeStatusFilter !== "alle") || (currentView === "beschwerden" && (haltestelleSearchTerm.trim() || linieSearchTerm.trim()))) ? ' und' : ''} im gewählten Zeitraum (${(dateFilterTarget === 'datum' && currentView === 'beschwerden') ? 'Vorfalldatum' : 'Erstellungsdatum'})`}
                        {!(searchTerm.trim() || emailSearchTerm.trim() || idSearchTerm.trim() || (currentView === "beschwerden" && (haltestelleSearchTerm.trim() || linieSearchTerm.trim())) || ((currentView === "beschwerden" || currentView === "lob" || currentView === "anregungen") && activeStatusFilter !== "alle") || (appliedStartDate || appliedEndDate)) && ` die Ansicht "${VIEW_TITLES[currentView]}"`}
                        gefunden.
                    </div>
                 ) : !isLoadingData && filteredData.length > 0 && !error ? (
                    <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-4 gap-5">
                        {filteredData.map((item) => (
                            <DataItemCard
                                key={`${currentView}-${item.id}`} item={item} currentView={currentView}
