...
export interface StatisticsData {
  ...
  complaintReasons?: ComplaintReason[]; // API kann leeres Array oder undefined senden
  averageProcessingTime: number | null;
  topComplaintLines?: ComplaintHotspot[]; // API kann leeres Array oder undefined senden
  topComplaintStops?: ComplaintHotspot[]; // API kann leeres Array oder undefined senden
  topComplaintTimes?: ComplaintHotspot[]; // API kann leeres Array oder undefined senden
  filterApplied?: { startDate?: string; endDate?: string; isDefault: boolean };
}

// --- Animationsvarianten und Hilfskomponenten (unverändert) ---
...
const CustomBarLabel = (props: any) => {     const { x, y, width, height, value, name } = props;    const labelText = `${name}: ${value}`;     const textX = x + width + 10;     const textY = y + height / 2;    if (width < 30 && value > 0) { return null; }    if (value === 0 || value === null || typeof value === 'undefined') return null;     return ( <g> <text x={textX} y={textY} dy={4} fill="#e2e8f0" fontSize={11} fontWeight={500} textAnchor="start" > {name} ({value}) </text> </g> );};
...
export default function StatisticsView() {
  ...
  const fetchStatistics = useCallback(async (startDate?: string | null, endDate?: string | null) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || errData.details || `Fehler ${response.status}`);
      }
      const apiData: StatisticsData = await response.json();

      // Fallback Dummy-Daten (nur für den Fall, dass die API die Felder gar nicht liefert UND kein Filter aktiv ist)
      // Normalerweise sollte die API immer alle Felder liefern, auch wenn sie leer sind.
      const isDateFilterApplied = !!(startDate || endDate);

      const defaultReasons: ComplaintReason[] = [ { reason: 'Fahrweise (z.B. zu schnell, abruptes Bremsen)', count: 22 }, { reason: 'Unpünktlichkeit / Verspätung', count: 18 }, { reason: 'Ausfall einer Fahrt / Zuges', count: 15 }, { reason: 'Verhalten des Fahrpersonals', count: 12 }, { reason: 'Sauberkeit im Fahrzeug/Haltestelle', count: 9 }, { reason: 'Tariffragen / Ticketkauf', count: 7 }, { reason: 'Informationen / Durchsagen', count: 6 }, { reason: 'Anschlusssicherheit', count: 5 }, { reason: 'Fahrzeugzustand (z.B. defekte Tür)', count: 4 }, { reason: 'Linienführung / Haltestellenposition', count: 3 }, ].sort((a,b) => b.count - a.count);
      const defaultTopLines: ComplaintHotspot[] = [ { name: 'Linie 3', count: 25 }, { name: 'Linie 1', count: 19 }, { name: 'Linie 4', count: 15 }, { name: 'Linie 2 (Nacht)', count: 12 }, { name: 'SEV Linie X', count: 8 }, { name: 'Linie 5', count: 7}, { name: 'Linie 16', count: 6}, { name: 'Linie 9', count: 5}, { name: 'Linie EV1', count: 4}, { name: 'Linie 10', count: 3}, ].sort((a,b) => b.count - a.count);
      const defaultTopStops: ComplaintHotspot[] = [ { name: 'Hauptbahnhof', count: 30 }, { name: 'Stadthalle', count: 22 }, { name: 'Thiemstr./Klinikum', count: 18 }, { name: 'Sandow', count: 14 }, { name: 'Sachsendorf', count: 10 }, { name: 'Sportzentrum', count: 9}, { name: 'Universität', count: 8}, { name: 'Zoo', count: 7}, { name: 'Schmellwitz Anger', count: 6}, { name: 'Madlow', count: 5}, ].sort((a,b) => b.count - a.count);
      const defaultTopTimes: ComplaintHotspot[] = [ { name: '07:00-09:00', count: 28 }, { name: '15:00-17:00', count: 24 }, { name: '12:00-14:00', count: 17 }, { name: '22:00-00:00', count: 13 }, { name: '09:00-11:00', count: 10 }, { name: '17:00-19:00', count: 9}, { name: '06:00-07:00', count: 8}, { name: '19:00-21:00', count: 7}, { name: '11:00-12:00', count: 6}, { name: '14:00-15:00', count: 5}, ].sort((a,b) => b.count - a.count);

      setStatsData({
          ...apiData, // Beginne mit den Daten von der API
          complaintsByStatus: apiData.complaintsByStatus?.map(item => ({...item, status: item.status as ChartComplaintStatusType})) || [],
          // KORREKTUR: Dummy-Daten nur verwenden, wenn API nichts liefert UND kein Filter aktiv ist
          complaintReasons: (isDateFilterApplied && apiData.complaintReasons) // Wenn Filter aktiv, nimm API-Daten (auch wenn leer)
                            ? apiData.complaintReasons.slice(0, 10)
                            : (apiData.complaintReasons && apiData.complaintReasons.length > 0
                                ? apiData.complaintReasons
                                : defaultReasons).slice(0, 10),
          topComplaintLines: (isDateFilterApplied && apiData.topComplaintLines)
                            ? apiData.topComplaintLines.slice(0, 10)
                            : (apiData.topComplaintLines && apiData.topComplaintLines.length > 0
                                ? apiData.topComplaintLines
                                : defaultTopLines).slice(0, 10),
          topComplaintStops: (isDateFilterApplied && apiData.topComplaintStops)
                            ? apiData.topComplaintStops.slice(0, 10)
                            : (apiData.topComplaintStops && apiData.topComplaintStops.length > 0
                                ? apiData.topComplaintStops
                                : defaultTopStops).slice(0, 10),
          topComplaintTimes: (isDateFilterApplied && apiData.topComplaintTimes)
                            ? apiData.topComplaintTimes.slice(0, 10)
                            : (apiData.topComplaintTimes && apiData.topComplaintTimes.length > 0
                                ? apiData.topComplaintTimes
                                : defaultTopTimes).slice(0, 10),
          // averageProcessingTime wird direkt von der API übernommen (kann null sein)
          filterApplied: {
      ...
      // Bei Fehler leere Daten setzen, damit keine Dummy-Daten angezeigt werden
      setStatsData(prev => ({
        ...(prev || { // Behalte totale Zahlen, falls sie vorher geladen wurden
            totalComplaints: 0, totalPraises: 0, totalSuggestions: 0,
            complaintsByStatus: [], complaintsOverTime: [], averageProcessingTime: null
        }),
        complaintReasons: [], topComplaintLines: [], topComplaintStops: [], topComplaintTimes: [],
        filterApplied: { startDate: startDate || undefined, endDate: endDate || undefined, isDefault: !startDate && !endDate }
      }));
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    ...
    >
      <motion.section variants={sectionVariants} className={`${glassEffectBaseClasses} p-5 sm:p-6 rounded-2xl relative`}> {/* relative hinzugefügt */}
        ...
        <div className="flex flex-wrap items-end gap-4 md:gap-6">
          <div className="flex-grow sm:flex-grow-0">
            ...
            <input
              type="date" id="startDate" value={selectedStartDate} onChange={(e) => setSelectedStartDate(e.target.value)}
              max={selectedEndDate || undefined}
              className="bg-slate-700/80 text-slate-100 border border-slate-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-sky-500 focus:border-sky-500 w-full sm:w-auto shadow-inner"
          ...
          <div className="flex-grow sm:flex-grow-0">
            ...
            <input
              type="date" id="endDate" value={selectedEndDate} onChange={(e) => setSelectedEndDate(e.target.value)}
              min={selectedStartDate || undefined} max={new Date().toISOString().split('T')[0]}
              className="bg-slate-700/80 text-slate-100 border border-slate-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-sky-500 focus:border-sky-500 w-full sm:w-auto shadow-inner"
              style={{ colorScheme: 'dark' }}
            />
          </div>
          <motion.button
            ...
            className="px-5 py-2.5 bg-sky-600 hover:bg-sky-500 text-white font-semibold rounded-lg shadow-md hover:shadow-lg transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
            whileHover={{ scale: isLoading ? 1 : 1.03 }} whileTap={{ scale: isLoading ? 1 : 0.97 }}
          >
            <FilterIcon size={16} /> Filter anwenden
          </motion.button>
          {isDateFilterActive && (
            <motion.button
              onClick={handleClearDateFilter} disabled={isLoading}
              className="px-4 py-2.5 bg-slate-600 hover:bg-slate-500 text-white font-semibold rounded-lg shadow-md hover:shadow-lg transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              whileHover={{ scale: isLoading ? 1 : 1.03 }} whileTap={{ scale: isLoading ? 1 : 0.97 }}
              title="Datumsfilter zurücksetzen"
            >
              <XMarkIcon size={16} /> Zurücksetzen
            </motion.button>
          )}
        </div>
        {error && error.includes("Startdatum") && (
             <p className="text-xs text-red-400 mt-2">{error}</p>
        )}
        {statsData?.filterApplied && !statsData.filterApplied.isDefault && statsData.filterApplied.startDate && statsData.filterApplied.endDate && (
            <p className="text-xs text-sky-300 mt-3">
                Statistiken für den Zeitraum: {formatDate(statsData.filterApplied.startDate)} - {formatDate(statsData.filterApplied.endDate)}
            </p>
        )}
         {statsData?.filterApplied && statsData.filterApplied.isDefault && (
            <p className="text-xs text-slate-400 mt-3">
                Standardansicht (z.B. Gesamt, letzte 30 Tage). Wählen Sie einen Zeitraum für spezifische Analysen.
            </p>
        )}
      </motion.section>

      {/* --- Bestehende Sektionen (Gesamtübersicht, Beschwerde-Analyse, Weitere Details, Hotspots, Berichte & Export) --- */}
      ...
      <motion.section variants={sectionVariants}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-6">
          ...
          <ChartWrapper title={statsData?.filterApplied?.isDefault === false ? `Im Zeitraum` : `Letzte 30 Tage`} isLoading={isLoading} error={error} icon={CalendarRange}>
      ...
      <motion.section variants={sectionVariants}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
            <ChartWrapper title="Top 10 Beschwerdegründe" isLoading={isLoading} error={error} icon={FileText} className="lg:col-span-2" minHeightClass="min-h-[480px] sm:min-h-[550px]">
                {statsData?.complaintReasons && statsData.complaintReasons.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart layout="vertical" data={statsData.complaintReasons} margin={{ top: 5, right: 30, left: 10, bottom: 20 }} >
                            ...
                            <XAxis type="number" allowDecimals={false} stroke={COLORS.slate} fontSize={10} tickLine={false} axisLine={{stroke: COLORS.slate, strokeOpacity:0.3}} />
                            <YAxis type="category" dataKey="reason" stroke={COLORS.slate} fontSize={11} tickLine={false} axisLine={false} width={220} tick={{ dx: -5, fill: '#cbd5e1' }} />
                            <Tooltip cursor={{ fill: 'rgba(14, 165, 233, 0.05)' }} content={<CustomTooltip />} />
            ...
            <StatCard title="Ø Bearbeitungszeit" value={isLoading ? '-' : (statsData?.averageProcessingTime !== null && typeof statsData?.averageProcessingTime !== 'undefined' ? statsData.averageProcessingTime : 'N/A')} unit={statsData?.averageProcessingTime !== null && typeof statsData?.averageProcessingTime !== 'undefined' ? "Tage" : ""} icon={Clock} />
        </div>
      </motion.section>
              ...
              {chartInfo.data && chartInfo.data.length > 0 ? (
                 <ResponsiveContainer width="100%" height="100%">
                   <BarChart data={chartInfo.data} layout="vertical" margin={{ top: 5, right: 45, left: 5, bottom: 5 }}>
                      ...
                      <Bar dataKey="count" name="Anzahl" radius={[0, 6, 6, 0]} barSize={chartInfo.barSize} fill={COLORS.amber} fillOpacity={0.9} isAnimationActive={true} animationDuration={RECHARTS_ANIMATION_DURATION} animationEasing={RECHARTS_ANIMATION_EASING}>
