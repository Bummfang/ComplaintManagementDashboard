...
import {
  ...
  TooltipProps
  // PieChart, Pie, Sector wurden entfernt, da renderActiveShape nicht mehr verwendet wird
} from 'recharts';
...
// renderActiveShape wurde entfernt, da nicht mehr verwendet.
// CustomBarLabel wurde entfernt, da nicht mehr verwendet.

const CustomTooltip = ({ active, payload, label }: TooltipProps<number, string>) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-slate-800/90 backdrop-blur-md p-3 rounded-lg shadow-xl border border-slate-100/10">
        <p className="text-sm text-sky-300 font-semibold mb-0.5">{label}</p>
        {payload.map((pld, index) => (
          <div key={index} style={{ color: pld.color || COLORS.sky }}>
            <span className="text-xs text-slate-200">{`${pld.name || 'Wert'}: `}</span>
            <span className="text-xs font-semibold">{pld.value}</span>
          </div>
        ))}
      </div>
    );
...
export default function StatisticsView() {
  ...
  // activeIndexPie und onPieEnter wurden entfernt.
  ...
  const fetchStatistics = useCallback(async (startDate?: string | null, endDate?: string | null) => {
    ...
    try {
      ...
      setStatsData({
        ...
        complaintReasons: (isDateFilterApplied && apiData.complaintReasons)
